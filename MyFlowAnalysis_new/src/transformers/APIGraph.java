...
public class APIGraph {
	...
	public List<APIGraphNode> getConditionalPredecessors(APIGraphNode node)
	{
		...
		preDecessors.removeAll(toExclude);
		return preDecessors;
	}
	...
	public void buildAPIGraph(SootMethod method){
		
		initAPIGraphFromCFG(method);
		//removeNonInvokeNode();
		//drawDataDependency();
		annotateGraph();
	}
	...
	*/
	...
	private void initAPIGraphFromCFG(SootMethod method){
		...
		while(iterCFG1.hasNext()){
			...
			APIGraphNode node = new APIGraphNode(s, method);
	 ...
	 * Keep the 'callsite' in NLP project
	 */
	public void inline(APIGraphNode callsite, List<APIGraph> callees)
	{
		//added for NLP project
		Vector<APIGraphNode> callsiteSuccs = callsite.getSuccessors();
		...
		for(APIGraph callee : callees)
		{
			if(!callee.apiGraph.isEmpty())
			{
				...
				//add successors of 'callsite' as the successors of 'callee'
				for(APIGraphNode callsiteSucc: callsiteSuccs)
				{
					for(APIGraphNode exitPoint : exitPoints)
					{
						exitPoint.addSucc(callsiteSucc);
						callsiteSucc.addPred(exitPoint);
					}
					callsiteSucc.removePred(callsite);
				}
				
				callsite.clearSucc();
				//add 'callsite' as predecessor of 'callee'
				for(APIGraphNode entryPoint : entryPoints)
				{
					entryPoint.addPred(callsite);
					callsite.addSucc(entryPoint);
				}
			}
		}
		
		/**
		 * This part of code is the original inline()
		 **/
		/*
		Vector<APIGraphNode> callsitePreds = callsite.getPredecessors();
		Vector<APIGraphNode> callsiteSuccs = callsite.getSuccessors();
		
		for(APIGraph callee : callees){
			if(callee.apiGraph.isEmpty()){
				for(APIGraphNode predNode : callsitePreds){
					for(APIGraphNode succ : callsiteSuccs){
						predNode.addSucc(succ);
					}
				}
				
				for(APIGraphNode succNode : callsiteSuccs){
					for(APIGraphNode pred : callsitePreds){
						succNode.addPred(pred);
					}
				}
			}
