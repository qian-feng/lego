...
public class GlobalBackwardDataflowAnalysis extends SceneTransformer {
	...
	public static Map<String, JimpleBody> methodTObody = new HashMap<String, JimpleBody>();
	...
	protected void internalTransform(String string, Map map){
		System.out.println("WITHIN GLOBALBACKWARDS");
		...
		if(AnalyzerMain.dataFlowForCondition == false)
		{
			Set<Stmt> sKeySet = this.stmtToNodeMap.keySet();
			System.out.println("PRITNTING STMTs");
		...
		if(AnalyzerMain.dataFlowForCondition == false)
		{
			//dumpDDGEdgeListToDot(this.apiDDGGraph, DDG_GLOBAL_SUCCINCT_DOT, false);
		...
		while (classes_iter.hasNext()) {
			...
			//System.err.println(soot_class.getName().toString());
			List<SootMethod> methods = soot_class.getMethods();
			for (SootMethod method : methods) {	
				if(!method.isConcrete()){
					continue;
				}
				//System.err.println(method.getName().toString());
				JimpleBody body = (JimpleBody) method.retrieveActiveBody();
				methodTObody.put(soot_class.getName().toString()+":"+method.getName().toString(), body);
			}
		}		
	}
	...
	/*private void RemoveNonImportantNodes() 
	{
		...
		if(AnalyzerMain.dataFlowForCondition == true)
		{
	...
	private void doDataFlowAnalysis() 
	{
		...
		while(sinkIter.hasNext()){
			...
			System.out.println("WITHIN doDATAFLOWANALYSIS");
			System.out.println(flowSink);

			LinkedHashMap<String, String> entryPointsString = AnalyzerMain.sourcesLocationMap.get(flowSink);
			System.out.println("PRINTING ENTRYPOINTSTRING: ");
			Iterator k = entryPointsString.entrySet().iterator();
		    while (k.hasNext()) {
			...
			while (keyIterator.hasNext()) {
				...
				if(conditionSig.isEmpty())
				{
					conditionSig = flowSink + "|" + mClass;
					//System.err.println("conditionSig in DFA: " + conditionSig);
				}
				
				stmtMethod = mClass.substring(mClass.indexOf("|") + 1);
				mClass = mClass.substring(0, mClass.indexOf("|"));
				
				System.out.println("METHOD IS:" +stmtMethod );
				System.out.println("CLASS IS:" +mClass );
			...
			while(!worklist.isEmpty()){
				...
				//identify "sources" in current method
				{
					...
					while (it.hasNext()) {
						...
						//added for NLP project. do data-flow analysis for all the variables in conditional statement
						if((s instanceof IfStmt || s instanceof TableSwitchStmt || s instanceof LookupSwitchStmt)
								&& s.toString().equals(flowSink)
								&& sMethod.getSignature().equals(stmtMethod))
						{
							
							//System.err.println("location1: " + s);
						...
						else
						{
							...
							while (useIt.hasNext()) {
								...
								}//end if (vBox.getValue() instanceof InvokeExpr)
							}//end while
						}
				...
				while(!usesStack.isEmpty()){
					...
					*/
					...
					else if((s instanceof IfStmt || s instanceof TableSwitchStmt || s instanceof LookupSwitchStmt) 
							&& s.toString().equals(flowSink)
							&& sMethod.getSignature().equals(stmtMethod))
					{
			...
			*/
			...
			//if this DFA is for condition, do the following
			if(AnalyzerMain.dataFlowForCondition == true)
			{
				Set<Stmt> sKeySet = this.stmtToNodeMap.keySet();
				Iterator<Stmt> sIter = sKeySet.iterator();
				...
				if(AnalyzerMain.DEBUG_CONDITION)
				{
					System.err.println("flowSink is: " + flowSink);
					printDDGEdgeList(this.apiDDGGraph);
				}
	...
	private void annotateAPIs(){
		...
		while (classes_iter.hasNext()) {
			...
			for (SootMethod method : methods) {	
				...
				//if(method.getName().toString().contains("turnR")) AnalyzerMain.mBody = body;
				//System.err.println(method.getName().toString());
	...
	*/
	...
	private static void removeNonAPIorConstNode(List<APIGraphNode> apiGraph){
		...
		for(APIGraphNode node : apiGraph){
			...
			//here we don't keep the statement which contains string constant. for NLP
	...
	private static void removeSpecificAPINode(List<APIGraphNode> apiGraph, List<String> classFilter, List<String> classPreserveSet){
		...
		for(APIGraphNode node : apiGraph){
			...
			//keep the statements which have string constant, disabled in NLP project
